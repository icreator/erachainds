...
public class Ed25519 {
    ...
    public static Pair<byte[], byte[]> createKeyPair(byte[] seed) {
        byte[] private_key = new byte[64]; // V
      //  byte[] private_key2 = new byte[64]; // V
        byte[] public_key = new byte[32];

        ge_p3 A = new ge_p3();

        sha512(seed, 32, private_key);
      //  sha512(seed, 64, private_key2);
        // ???
     //   LOGGER.debug("after sha 512:     " + Base58.encode(private_key));
    //    LOGGER.debug("after sha 512:     " + Base58.encode(private_key2));
        private_key[0] &= 248;
        private_key[31] &= 63;
        private_key[31] |= 64;
        // 5y5r1azV4h4cxjUPByo6C3LikJY8v5PPWuX5CxvEd3oGYdX78dLErRc5Cxq3HievXNJaLLFX75329ssKa2MfKuG3
        //LOGGER.debug("before scalarmult: " + Base58.encode(private_key));

        ge_scalarmult_base.ge_scalarmult_base(A, private_key);
        ge_p3_tobytes.ge_p3_tobytes(public_key, A);
       // LOGGER.debug("after scalarmult:  " + Base58.encode(private_key));
